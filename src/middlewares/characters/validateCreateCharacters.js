...
function validateCreateCharacters(req, res, next) {
	...
	if (req.body.history?.length > 500) {
		return res.status(HttpStatusCode.BAD_REQUEST).json(makeResponse(null, false, "history is required. ", "history is missing."));
	}

	next()
}

module.exports = validateCreateCharacters
